...
const Slide = ({ data }: ISlideProps) => {
  return (
    <div className='grid grid-rows-2 grid-cols-3 place-items-center gap-2 w-72 border-2 bg-gray-400'>
      {data.map((el) => (
        <div
    ...
    <section className='w-full h-full flex flex-col items-center'>
      <div className='w-full h-1/2 flex justify-around items-center'>
        <button onClick={goLeft}>
          <FaChevronLeft color='#5965F4' size={'30px'} />
        </button>
        <div className='w-72 h-full flex overflow-x-hidden'>
          <div
        ...
        <button onClick={goRight}>
          <FaChevronRight color='#5965F4' size={'30px'} />
        </button>
      </div>
      <div className='w-full h-6 flex justify-center gap-1 mt-5'>
        {dotsArr.map((dot) => (
          <span key={dot} onClick={() => setCurIndex(dot)}>
            {dot === curIndex ? (
              <div className='w-2 h-2 rounded-full bg-[#5965F4]'></div>
            ) : (
              <div className='w-2 h-2 rounded-full bg-[#C7C7C7]'></div>
            )}
          </span>
        ))}
      </div>
